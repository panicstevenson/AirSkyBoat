...
xi.magic.applyResistanceEffect = function(caster, target, spell, params)
    ...
    local p = xi.magic.getMagicHitRate(caster, target, skill, element, effectRes, magicaccbonus, nil, skillchainCount, false, params.hybridHit)

    return xi.magic.getMagicResist(p, target, element, effectRes, skillchainCount, effect, caster, false)
end

-- Applies resistance for additional effects
xi.magic.applyResistanceAddEffect = function(player, target, element, effect, bonus, hybridHit)
    local effectRes = 0
...
-- luacheck: ignore 561
xi.magic.getMagicHitRate = function(caster, target, skillType, element, effectRes, bonusAcc, dStat, skillchainCount, isDamageSpell, hybridHit)
    local magicacc = 0
    local magiceva = 0
    local resMod = 0
    ...
    return xi.magic.calculateMagicHitRate(magicacc, magiceva, target, element, skillchainCount, skillType, caster, isDamageSpell, hybridHit, dLvl)
end
