...
xi.spells.healing.handleDayWeatherBonus = function(caster, bonus, iridescence, element)
    local dwbonus = 1

    if iridescence and bonus > 0 then
        if math.random() < 0.33 or caster:getMod(xi.magic.elementalObi[element]) then
            dwbonus = dwbonus + 0.10
        end
    end

    if math.random() < 0.33 or caster:getMod(xi.magic.elementalObi[element]) then
        dwbonus = dwbonus + bonus
    end
...
xi.spells.healing.getDayWeatherBonus = function(caster, element)
    ...
    local iridescence    = caster:getMod(xi.mod.IRIDESCENCE) >= 1
    ...
    return xi.spells.healing.handleDayWeatherBonus(caster, weatherBonus, iridescence, element)
end
