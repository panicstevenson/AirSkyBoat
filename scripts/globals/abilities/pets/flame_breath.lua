...
abilityObject.onUseAbility = function(pet, target, skill, action)
    ...
    local dmgmod = xi.mobskills.mobBreathMove(pet, target, 0.185, pet:getMainLvl() * 15, xi.magic.ele.FIRE) -- Works out to (hp/6) + 15, as desired
    dmgmod = (dmgmod * (1 + gear)) * deep
    pet:setTP(0)

    local dmg = AbilityFinalAdjustments(dmgmod, pet, skill, target, xi.attackType.BREATH, xi.damageType.FIRE, xi.mobskills.shadowBehavior.IGNORE_SHADOWS)
    target:takeDamage(dmg, pet, xi.attackType.BREATH, xi.damageType.FIRE)

    target:addStatusEffectEx(xi.effect.ELEMENTAL_EVASION_DOWN, xi.effect.NONE, 2, 0, 15, 1, 0, 0, xi.effectFlag.NO_LOSS_MESSAGE, true)
