...
entity.onTrigger = function(player, npc)
    -- One cookie for the entire event
    if xi.events.starlightCelebration.isStarlightEnabled ~= 0 then
        if player:getCharVar("YouGotYourCookie") ~= 1 then
            player:PrintToPlayer("Sorry the teleports are not in service this year, here is a delicious cookie!", 29)
            player:addItem(4394, 1)
            player:messageSpecial(ID.text.ITEM_OBTAINED, 4394)
            player:setCharVar("YouGotYourCookie", 1)
        else
            player:PrintToPlayer("Merry Christmas and Happy Holidays!", 29)
        end
    end
    -- return
    -- if xi.events.starlightCelebration.isStarlightEnabled ~= 0 then
