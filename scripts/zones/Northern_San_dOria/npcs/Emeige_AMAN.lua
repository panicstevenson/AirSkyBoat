...
entity.onTrigger = function(player, npc)
    ...
    if not player:getMentor() then
        ...
        if player:getMainLvl() >= 30 and player:getPlaytime() >= 648000 then
            ...
            player:timer(8500, function(playerArg) playerArg:PrintToPlayer("Mentor status can also be switched on and off in the \"Config\" menu found within the \"Help Desk.\"", xi.msg.channel.SAY, npc:getPacketName()) npc:setRotation(rot) end)
            ...
            player:timer(7000, function(playerArg) playerArg:PrintToPlayer("Once you have met the requirements to become a mentor, please speak with me again.", xi.msg.channel.SAY, npc:getPacketName()) npc:setRotation(rot) end)
        end
    elseif player:getMentor() then
        player:startEvent(739, 10)
    end
end
