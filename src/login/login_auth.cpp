        ...
        switch (code)
        {
            ...
            case LOGIN_CREATE:
            {
                ...
                else
                {
                    ...
                    if (sql->NumRows() == 0)
                    {
                        ...
                        ShowInfo(fmt::format("login_parse: account<%s> was created", escaped_name));
                        sessions[fd]->wdata.resize(1);
                    ...
                    {
                        ShowWarning(fmt::format("login_parse: account<%s> already exists", escaped_name));
                        sessions[fd]->wdata.resize(1);
            ...
            case LOGIN_CHANGE_PASSWORD:
            {
                ...
                else
                {
                    ...
                    if (ret == SQL_ERROR || sql->NumRows() == 0)
                    {
                        ShowWarning(fmt::format("login_parse: user <%s> could not be found using the provided information. Aborting.", escaped_name));
                    }
                    ...
                    if (status & ACCOUNT_STATUS_CODE::BANNED)
                    {
                        ...
                        ShowInfo(fmt::format("login_parse: banned user <%s> detected. Aborting.", escaped_name));
                        do_close_login(sd, fd);
                        return 0;
                    }
                    ...
                    if (status & ACCOUNT_STATUS_CODE::NORMAL)
                    {
                        ...
                        if (size == 0 || size > 16)
                        {
                            sessions[fd]->wdata.resize(1);
                            ref<uint8>(sessions[fd]->wdata.data(), 0) = LOGIN_ERROR_CHANGE_PASSWORD;
                            ShowWarning(fmt::format("login_parse: Invalid packet size (%d). Could not update password for user <%s>.", size,
                                        escaped_name));
                            do_close_login(sd, fd);
                            return 0;
                        }
                        ...
                        if (ret == SQL_ERROR)
                        {
                            sessions[fd]->wdata.resize(1);
                            ref<uint8>(sessions[fd]->wdata.data(), 0) = LOGIN_ERROR_CHANGE_PASSWORD;
                            ShowWarning(fmt::format("login_parse: Error trying to update password in database for user <%s>.", escaped_name));
                            do_close_login(sd, fd);
                            return 0;
                        }
