...
#define MAX_POINTS 10000
    ...
    void SetContestStatus(uint8 newStatus, bool onLoad, bool isTest)
    {
        ...
        switch ((FISHINGCONTESTSTATUS)newStatus)
        {
            case CONTEST_STATUS_CONTESTING:
                if (!onLoad || FishingContest.startTime == 0)
                {
    ...
    fish_ranking_entry* GetFishRankEntry(uint8 entry)
    {
        ...
        if (FishingContest.status < (uint8)CONTEST_STATUS_ACCEPTING) // This should always be an empty vector if we haven't accepted fish yet!
        {
            return nullptr;
        }
        ...
        else if (!FishingContestEntries.empty() && entry >= FishingContestEntries.size())
        {
            ...
            {
                step      = 1;
                fakeScore = 9984; // Allows for max of 40 sequenced scores capping at 9999 or 1 depending on direction
            }

            fakeScore += (entry * step);
            ...
            fakeEntry->mjob        = ((fakeCounter - 1) % MAX_JOBTYPE) + 1;
            fakeEntry->sjob        = 0;
    ...
    fish_ranking_entry* GetPlayerEntry(CCharEntity* PChar)
    {
        ...
        {
            // Set the rank value
            if (!strcmp(PChar->name.c_str(), it->name))
            {
                return it;
            }
        }
    ...
    /***********************************************
     *  Submit a fish for contest ranking
     *  Returns 0 on success, 1 if fish is already ranked, 2 if player already has a fish submitted
     *
     ***********************************************/
    void SubmitFish(CCharEntity* PChar, uint32 score, bool isReplacement)
    {
        // You should only be able to do this when the contest is in ACCEPTING mode, but keeping that check out of core for customization purposes.
        // Note: Checking for a fish having been ranked already should be done in Lua

        // Placeholder for the entry
        fish_ranking_entry* entry = nullptr;

        // Check to see if the player has already submitted a fish
        fish_ranking_entry* original = nullptr;
        for (int it = 0; it < (int)FishingContestEntries.size(); it++)
        {
            if ((std::string)FishingContestEntries[it]->name == PChar->name)
            {
                if (isReplacement)
                {
                    original = FishingContestEntries[it];
                    break;
                }
                else
                {
        ...
        if (original)
        {
            // Do a replacement in the table
            strcpy(original->name, PChar->name.c_str()); // Remember to truncate this to 16 chars max before sending packet
            original->mjob        = (uint8)PChar->GetMJob();
            original->sjob        = (uint8)PChar->GetSJob();
            original->mlvl        = PChar->GetMLevel();
            original->slvl        = PChar->GetSLevel();
            original->race        = PChar->mainlook.race;
            original->allegiance  = (uint8)PChar->allegiance;
            original->fishrank    = PChar->RealSkills.rank[SKILLTYPE::SKILL_FISHING];
            original->score       = score;
            original->submittime  = CVanaTime::getInstance()->getVanaTime();
            original->contestrank = 0; // Not set until contest closes.
        ...
        else
        {
            ...
            entry->contestrank = 0; // Not set until contest closes.
    ...
    /***********************************************
     *  Return the score of a fish given base on contest rules
     *
     *
     ***********************************************/
    ...
    bool HasRewardPending(CCharEntity* PChar, uint16 contestId)
    {
        ...
        const char* Query = "SELECT r.time "
        ...
        int32 ret = sql->Query(Query, contestId, PChar->GetName());
        if (ret != SQL_ERROR)
        {
     ...
     *  CALLED BY THE TIME_SERVER!  DO NOT FILL WITH ADDITIONAL CALLS
     ***********************************************/
        ...
        if (currentTime > FishingContest.changeTime)
        {
            switch ((FISHINGCONTESTSTATUS)FishingContest.status)
            {
                ...
                case CONTEST_STATUS_PRESENTING:
                    ...
                    break;
                case CONTEST_STATUS_HIATUS:
                    InitNewContest(FishingContest.contestId + 1);
                    break;
    ...
    void InitNewContest(uint16 contestId, uint32 fishId, uint8 criteria, uint8 measure)
    {
        ...
        SetContestStatus(CONTEST_STATUS_CONTESTING, false);
    ...
    void ScoreContest()
    {
        ...
        for (fish_ranking_entry* it : FishingContestEntries)
        {
            ...
            if (score == 0)
            {
                it->contestrank = contestrank; // This is the first one, so nothing to compare to
                score           = it->score;   // But now we are setting the bar
            }
            else
            {
                if (it->score != score) // Regardless of increasing or decreasing ... Vector already sorted
                {
                    contestrank += rankStep;
            ...
            it->dataset_a   = std::count_if(FishingContestEntries.begin(), FishingContestEntries.end(),
                                            [score](fish_ranking_entry* a) -> bool
                                            {
                                              return a->score == score;
                                          });
            // clang-format on
            it->dataset_b   = it->dataset_a;
            it->resultcount = (uint8)FishingContestEntries.size();
        }

        // Contest is closed at this point and nothing else should be added to it
    ...
    void LoadCurrentContest()
    {
        ...
        const char* Query = "SELECT "
                            ...
                            "INNER JOIN (SELECT MAX(contestid) maxid "
                            "FROM `fishing_contest`) m "
                            "ON m.maxid = f.contestid;";
        ...
        else if (ret != SQL_ERROR && sql->NumRows() == 0)
        {
            ...
            FishingContest.contestId = 1;
            auto getRandomFish       = lua["xi"]["fishing"]["contest"]["randomFish"];
            if (!getRandomFish.valid())
            {
                sol::error err = getRandomFish;
                ShowError("fishingutils:SetContestStatus: %s", err.what());
            }
            auto result = getRandomFish();
            if (!result.valid())
            {
                sol::error err = result;
                ShowError("fishingutils:SetContestStatus:(getRandomFish): %s", err.what());
            }
            uint8 criteria = (uint8)std::floor(xirand::GetRandomNumber(0, 3));
            uint8 measure  = (uint8)std::floor(xirand::GetRandomNumber(0, 2));

            InitNewContest(1, (uint32)result, (FISHINGCONTESTCRITERIA)criteria, (FISHINGCONTESTMEASURE)measure);
            ShowWarning("No Fishing Contest found in database.  Initializing new one.");
        }
    ...
    void LoadContestEntries(uint8 contest)
    {
        ...
        int32 ret = sql->Query(Query, contest);
